    ...
    <div>
      <header className="w-full h-[100vh] relative">
        ...
        <div className="absolute inset-x-0 top-4 grid px-6 xl:px-8 h-[80px] xl:h-[120px]">
          ...
          <motion.nav className={`fixed ${window.innerWidth >= 1280 ? "hidden" : "flex"} top-0 z-20 bg-white/50 backdrop-blur-xl min-w-[300px] max-w-[500px] h-full xl:flex xl:space-x-32 text-lg xl:text-3xl right-0 transition-transform duration-300 ${isNavOpen ? "transform translate-x-0" : "transform translate-x-full"}`} initial={false} animate={isNavOpen ? "end" : "start"} transition={{ delayChildren: 0.05, staggerChildren: 0.1 }}>
